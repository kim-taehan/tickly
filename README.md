...
설치된 앱은 시작 시(및 4시간마다) GitHub Release를 확인해 새 버전이 있으면 자동으로 내려받고 다음 종료 시 설치합니다.
- **Windows**: 무인 자동 업데이트 동작.
- **macOS**: 다운로드는 되지만 설치 적용에는 코드 서명이 필요합니다(미서명 시 수동 재설치).

배포자는 버전을 올리고 태그를 푸시하면 CI가 전 플랫폼을 빌드·게시합니다:

```bash
# package.json version 수정 후
git tag v0.0.2 && git push origin v0.0.2   # → .github/workflows/release.yml 트리거
```
